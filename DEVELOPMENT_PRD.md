...
*   **跳转协议 (核心变更)**：
    *   **方案 A**：所有内置小程序跳转必须采用**后端动态获取 URL Scheme** 模式。
    *   **前端逻辑**：点击按钮 -> 调用 API -> 后端请求微信 `generatescheme` -> 前端接收并 `window.location.href`。
    *   **交互规范**：请求期间按钮需显示 `Loading` 状态，防止重复点击。
    *   **外部 H5**：标准 HTTPS 链接，直接新窗口打开。
...
> 2. **路线图模块**：实现多级树状图，点击节点进入详情页，详情页需包含基于**后端动态生成 URL Scheme** 协议的小程序跳转工具列表，并处理 Loading 状态。
